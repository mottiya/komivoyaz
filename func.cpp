...
void Komi::find_low_limit_normaliz()
{
	low_path.alloc(size());
	low_limit = 0;
	
	map< pair<size_t, size_t>, size_t> buff;
	pair<pair<size_t, size_t>, size_t> min_in;

	for (size_t i = 0; i < size(); i++)
	{
		create_matrix_str(buff, i);
		min_in = find_min_map(buff);
		low_limit += min_in.second;
		cout << ";low_limit = " << low_limit << ";";
		for (int j = 0; j < size(); j++) {
			if (matrix.at({ i, j }) == INT_MAX) continue;
			matrix[{i, j}] -= min_in.second;
		}
	}

	for (size_t i = 0; i < size(); i++)
	{
		create_matrix_colum(buff, i);
		min_in = find_min_map(buff);
		low_limit += min_in.second;
		cout << ";low_limit = " << low_limit << ";";
		for (int j = 0; j < size(); j++) {
			if (matrix.at({ j, i }) == INT_MAX) continue;
			matrix[{j, i}] -= min_in.second;
		}
	}
}

void Komi::find_best_edges()
{
